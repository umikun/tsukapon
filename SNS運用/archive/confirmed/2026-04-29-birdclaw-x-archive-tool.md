...
## 🚪 Phase B 完了記録（2026-04-30）— **Birdclaw撤退、vault直接統合に切り替え**

### 実測値が想定を覆した

| 項目 | 想定（L171等） | 実測 | ギャップ |
|---|---|---|---|
| ZIPサイズ | 5〜15GB | **16MB** | 約 **1/300** |
| ツイート総数 | 数千〜数万件 | **600件** | 約 1/10 〜 1/30 |
| アーカイブ到着 | 5/1〜5/2 | 2026-04-30 13:51 | 想定より早着（約24h） |

### 規模感を踏まえた再判断

**Birdclaw（FTS5+SQLite+WebUI）は完全にオーバースペック**と判定。代わりに **vault 直接統合**で同等以上の価値を達成：

- 600件・16MB なら Obsidian グローバル検索（`Cmd+Shift+F`）で FTS5 の代替が成立
- ポート3000衝突・WIP（v0.2.1）リスク・launchd常駐・OpenAI APIキー管理 — **これら慎重ポイントを全部回避**
- 既存 launchd→Google Drive 同期にバックアップが自動相乗り
- n8n 統合が必要になっても vault 内で完結するから設計がシンプル化（Phase C も再設計可能）

### 実施した作業（[[SNS運用/archive/x-history/README.md]] 参照）

```
SNS運用/archive/x-history/    （総1.2MB）
├── README.md                  ← 取り込みサマリー
├── tweets/
│   ├── tweets.jsonl           ← 全600件
│   ├── tweets-by-favorite.md  ← いいね Top30
│   ├── tweets-by-retweet.md   ← リポスト Top30
│   └── by-year/{2022,2023,2024,2026}.md
├── articles/                  ← X Articles 12件を個別MD化
├── likes/likes.jsonl          ← 956件
└── social/{followers,following}.jsonl
```

### 重要な発見（次回以降の運用に反映）

1. **Twitter公式アーカイブには `impression_count` が含まれない** — 調査ノート L154 の「imp/like/RT数も含まれる」は誤り。impression が必要なら週次分析CSV（`SNS運用/analytics/source/`）を別途参照する必要
2. **2025年が完全空白** — 運用本格化が2026年からであり、「過去の自分のリミックス」より「2026年内の動線整理」の方が運用上の価値が高い
3. **DM 0件** — `direct-messages.js` 自体は126KB存在するが、標準キーで取得できなかった。必要時は raw 処理が必要

### 残タスク（Phase C への引き継ぎ）

- [ ] 月1（または運用見合いで四半期1回）の再申請 → 同じスクリプトで上書き再生成
- [ ] Phase C（n8n × vault 月次パイプライン）の判断は **手動運用1ヶ月後の6月** に持ち越し
- [ ] [[SNS運用/analytics/フォロワー改善.md]] のコンテンツリサイクル戦略と `tweets-by-favorite.md` の上位を突き合わせる運用を試行

### 棚上げした項目

- ❌ Birdclaw インストール（オーバースペックのため不採用）
- ❌ launchd 常駐ジョブ（必要性が消えた）
- ❌ OpenAI APIスコアインボックス（マシン規模で不要）

→ **総評**: 調査ノートの慎重姿勢（「価値最高だが緊急度低い」L177）が正解だった。実測16MB が来たことで「Birdclaw自体不要」が明確化し、結果的に**最短経路でvault統合まで完了**。Phase B はクローズ、再申請月1運用に移行。

...
- ~~**自分のvaultに当てる検証ネタ**: `SNS運用/` 配下と `_ kiwami/` 配下と `調査/` 配下を一括Graphify化したら、**「自分が無意識に何を中心に考えているか」のgod node**が見える可能性。批評型ポジションの裏付けデータとして強い~~ → ⚪ **見送り判定（2026-04-29）**: vault規模実測 = SNS運用/ 384・_ kiwami/ 45・調査/ 10 で計439ファイル。Graphifyフル実行は$15〜50/初回、Phase 1（調査/のみ）でも$0.5〜2のClaude API課金が必要。さらに **Obsidian Graph View で god node候補は5分・¥0で見える**ため、Graphifyの追加価値（意味的クラスタ＋GRAPH_REPORT.md）が単独で見合うかは未検証。**現時点では実装しない**。再検討する場合は、Phase 0（Obsidian Graph Viewで god node確認）→ 不足を感じたらPhase 1（調査/のみ$2試運転）の順で。サブスクOAuth経由は ToS 違反なので必ずAPI Key課金前提

...
### 1. [[.claude/commands/re-daily.md|/re-daily]] — note記事リライト＋サムネ文言＋X記事版＋Xスレッド＋本日アクションプラン一気通貫生成 ⭐ UPDATED 2026-05-07（Step 7「サムネイル文言生成」のフォーマット変更：各見出しに**強調語**フィールドを追加し、**AI流し込み用コードブロック**と**5見出し一括生成用統合ブロック**を併記。ChatGPT/GPT-Image-2 等への直接コピペで背景画像5枚同時アップロード→テキスト配置生成が可能に）

> **2026-05-07 変更点（サムネイル文言の AI流し込み対応）**: 既存の人間用メタデータ箇条書きはそのまま維持しつつ、**画像生成AIへ直接ペーストできる構造化ブロック**を併記する設計に変更:
>
> - **強調語フィールド追加**: メインタイトル内の「ゴールド表示にする部分（1〜5字）」を必ず1つ選定。`"..."` `「...」` の囲み語 → ベネフィット語 → 数字・固有名詞 の優先順
> - **各見出し直下に「AI流し込み用（コピペ）」コードブロック**: `- メイン:` `- 強調（ゴールド）:` `- サブ:` の3行構造。ChatGPT/GPT-Image-2 にそのまま貼り付け可
> - **末尾に「🎯 5見出し一括生成用」統合ブロック**: 背景画像5枚同時アップロード時の `▼ 添付N枚目 → 見出しN` 形式。1リクエストで5枚生成を狙う運用に対応
> - **ファイルフォーマット例の外側フェンスは ```` (4バッククォート)** に変更（内側のAI流し込み用コードブロックとのネストを正しく表現）
> - 章数が5以外（4章・6章等）の場合は統合ブロックも実章数に合わせて増減
...
- **Step 1〜6**: note本文のリライト
  ...
  - タグ5つ選定
- **Step 7**: サムネイル文言を自動生成（`SNS運用/title/title-YYYYMMDD.md`） ⭐ UPDATED 2026-05-07
  - メインサムネ＋各見出しサムネの「メインタイトル（12〜20字）」「サブタイトル（20〜35字）」
  - 7つの型のいずれか＋可能なら関係性キーワード（上司／同僚／会社）
  - **強調語**（ゴールド表示部分・1〜5字）を必ず1つ選定
  - 各見出し直下に **AI流し込み用コードブロック**（`- メイン:` `- 強調（ゴールド）:` `- サブ:`）
  - ファイル末尾に **5見出し一括生成用統合ブロック**（背景画像5枚同時アップロード対応）
...
### 7. [[.claude/commands/deep-dive.md|/deep-dive]] — ペースト文章の深掘り調査ノート生成 ⭐ NEW 2026-04-23 / 🌐 **グローバル化済み 2026-04-30** / 🆕 **2026-05-06: 「🗒 メモ」セクションを末尾→冒頭（TL;DR直下）へ移動** / ✅ **2026-05-07: グローバル版（`~/.claude/commands/deep-dive.md`）にもメモ冒頭ルールを同期完了**（プロジェクト版・グローバル版どちらがロードされても挙動一致）/ 🆕 **2026-05-07: frontmatter に `action: {投稿ネタ｜運用参考｜サブスクネタ｜採用なし}` を必須化**（Bases / Dataview からの用途別横断抽出を実現）
...
- フロントマターに `created` / `tags` / `source` / `action` を自動付与
  - `action` は **`投稿ネタ｜運用参考｜サブスクネタ｜採用なし` の4値固定**（2026-05-07追加）。メモセクションの「このネタの使い道」記述を最優先で参照してClaudianが判定し、自動セット。Bases / Dataview から用途別に横断抽出するための分類タグ

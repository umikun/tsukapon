...
### 1. [[.claude/commands/re-daily.md|/re-daily]] — note記事リライト＋サムネ文言＋X記事版＋Xスレッド＋本日アクションプラン一気通貫生成 ⭐ UPDATED 2026-05-03（批評型リプ枠（朝09:30・昼13:00・夜19:30）をルーチン枠から削除。ユーザーが空き時間に随時対応する運用に変更したため、自動生成しないように Step 11-2-ter を改訂。ルーチン枠は 10本→7本固定に）

> **2026-05-03 変更点（批評型リプをスケジュールから除外）**: GW中等で外出が多く予定通り動けない日でも詰まらないよう、批評型リプはユーザーが空き時間に随時対応する運用へ:
>
> - **削除した枠**: 09:30 朝の批評型リプ／13:00 昼の批評型リプ／19:30 夜の批評型リプ（戦略メモ B項目に記載があっても本日アクションプランには起こさない）
> - **ルーチン枠を 10本 → 7本固定に縮小**
> - **休養日の最低エントリ数を 6本 → 3本に変更**（KPI＋note公開＋X記事転載版公開）
> - **KPI記録（21:00）の集計対象から批評リプを除外**（実行本数が日次でブレるため、週次で別集計）
> - 旧 11-2-ter（10本固定／批評リプ3枠込み）は廃止

> **2026-05-02 変更点（本日タイムラインの固定スケジュール化）**: アクションプランの時刻配置がブレないよう、Step 11-2-ter を「ルーチン枠 + 変動枠」の二層構造に再設計:
>
> - **A. ルーチン枠（毎日 7本固定・2026-05-03 改訂）**:
>   - **09:00** 🐦 day{N} 投稿①（朝）
...
> - **休養日でも最低3本** = KPI＋note公開＋X記事転載版公開（投稿系のみスキップ可）
...
### 7. [[.claude/commands/deep-dive.md|/deep-dive]] — ペースト文章の深掘り調査ノート生成 ⭐ NEW 2026-04-23 / 🌐 **グローバル化済み 2026-04-30** / 🆕 **2026-05-06: 「🗒 メモ」セクションを末尾→冒頭（TL;DR直下）へ移動**
...
5. 調査ノートの組み立て（**TL;DR ／ 🗒 メモ（冒頭・2026-05-06変更） ／ 元テキスト抜粋 ／ キーワード早見表 ／ 背景 ／ ファクトチェック表 ／ 最新動向 ／ 関連概念 ／ 反対意見 ／ 残る疑問 ／ 参考資料**の11セクション構成）。**メモ（活用仮説・SNS転用・自分の運用への応用）を冒頭に置くことで、調査ノートが「使われる」確率を上げる設計**
...
| [[SNS運用/note/_メンバーシップ準備ロードマップ.md]] ⭐ NEW 2026-05-03 | noteメンバーシップ開設準備の週次タスク表（W19後半〜W22のGo/No-Go判定までの工程） |
...
| [[SNS運用/archive/x-history/README.md]] ⭐ NEW 2026-04-30 | **X公式アーカイブ取り込み版**。tweets.jsonl（600件）/ いいね・リポストTop30 / 年次ダンプ / Articles12件 / likes.jsonl（956件）。Birdclaw撤退判断＋vault直接統合の最終形（→ [[2026-04-29-birdclaw-x-archive-tool]] Phase B 完了記録） |
...
**rsync同期との関係**: rsync同期は MEGA へのバックアップ（24h）、git push は GitHub への履歴管理。同じ vault に対して二重に冗長化されている。
...
### 🔄 自動バックアップ（launchd） ⭐ UPDATED 2026-05-02（同期先を Google Drive → **MEGA** に変更）

**内容**: Tsukapon vault を1時間ごとに MEGA へrsync同期
...
**同期先**: `/Volumes/500GB/MEGA/Tsukapon`（旧 `/Volumes/500GB/GoogleDrive/Tsukapon` は手動で削除するまで残置）
...
```
vault変更 → launchd（1時間ごと） → rsync → MEGA同期
                                ↘
...
| バックアップ先 | `/Volumes/500GB/MEGA/Tsukapon/` |

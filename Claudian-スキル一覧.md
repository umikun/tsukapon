...
| 完了ノートを `_fin/` `_archive/` へ仕分け＋時系列ログ化 | `archive` | `/archive フォルダパス` または `/archive`（対話） |
...
**用途**: 完了マーカー付きのノートを `_fin/` または `_archive/YYYY-MM/` に自動移動し、対象フォルダ直下の `archive.md` にタイムスタンプ付きの集約ログを追記する。Hermes Kanban v1.5.0 の「Smart Card Archive」発想をObsidian vault運用に持ち込んだもの。
...
2. 完了マーカーのスキャン（既に `_fin/` `_archive/` 配下のものは除外）
3. 移動先の決定（同階層に `_fin/` があれば優先、なければ `_archive/YYYY-MM/`）
...
**連携先**: 旧来の手動 `_fin/` 退避運用と互換（手作業を自動化する立ち位置）
